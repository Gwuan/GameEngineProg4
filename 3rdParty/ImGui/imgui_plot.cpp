    ...
    if (conf.values.count > 0) {
        ...
        if (conf.grid_x.show) {
            ...
            case PlotConfig::Scale::Linear: {
                ...
                float inc = 1.f / cnt;
                for (int i = 0; i <= cnt; ++i) {
                    float x0 = ImLerp<float>(inner_bb.Min.x, inner_bb.Max.x, i * inc);
            ...
            case PlotConfig::Scale::Log10: {
                ...
                while (start < x_max) {
                    for (int i = 1; i < 10; ++i) {
                        ...
                        float x0 = ImLerp<float>(inner_bb.Min.x, inner_bb.Max.x, t);
                        window->DrawList->AddLine(
                            ImVec2(x0, y0),
                            ImVec2(x0, y1),
        ...
        if (conf.grid_y.show) {
            ...
            float inc = 1.f / cnt;
            for (int i = 0; i <= cnt; ++i) {
                float y0 = ImLerp<float>(inner_bb.Min.y, inner_bb.Max.y, i * inc);
                window->DrawList->AddLine(
                    ImVec2(x0, y0),
                    ImVec2(x1, y0),
